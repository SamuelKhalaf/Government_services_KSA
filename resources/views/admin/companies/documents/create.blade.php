...
@section('title', __('companies.add_document') . ' - ' . $company->company_name_en)
        ...
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">{{ __('companies.add_document') }}</li>
                </ul>
                <!--end::Breadcrumb-->
            </div>
            <!--end::Page title-->
            ...
            <div class="card">
                ...
                <div class="card-header">
                    ...
                    <div class="card-title fs-3 fw-bold">
                        {{ __('companies.add_document') }} - {{ $company->company_name_en }}
                        <span class="badge badge-light-primary ms-2">{{ __('companies.company') }}</span>
                    </div>
                    <!--end::Card title-->
                </div>
                <!--end::Card header-->
                ...
                <!--begin::Form-->
                <form id="kt_company_document_form" class="form" action="{{ route('admin.companies.documents.store', $company) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <!--begin::Card body-->
                    <div class="card-body p-9">
                            ...
                            <div class="col-xl-9 fv-row">
                                ...
                                <div class="row g-9 mb-8">
                                    ...
                                    <div class="col-md-6 fv-row">
                                        <label class="required fs-6 fw-semibold mb-2">{{ __('common.document_type') }}</label>
                                        <select class="form-select form-select-solid @error('document_type_id') is-invalid @enderror"
                                            ...
                                            <option value="">{{ __('companies.select_document_type') }}</option>
                                    ...
                                    <!--begin::Col-->
                                    <div class="col-md-6 fv-row">
                                        <label class="required fs-6 fw-semibold mb-2">{{ __('common.status') }}</label>
                                        <select class="form-select form-select-solid @error('status') is-invalid @enderror" name="status" required>
                                            <option value="active" @selected(old('status', 'active') == 'active')>{{ __('common.active') }}</option>
                                            <option value="expired" @selected(old('status') == 'expired')>{{ __('common.expired') }}</option>
                                            <option value="cancelled" @selected(old('status') == 'cancelled')>{{ __('common.cancelled') }}</option>
                                            <option value="pending" @selected(old('status') == 'pending')>{{ __('common.pending') }}</option>
                                        </select>
                                        @error('status')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                        ...
                        <div id="dynamic_fields_section" style="display: none;">
                            <div class="row mb-8">
                                <!--begin::Col-->
                                <div class="col-xl-3">
                                    <div class="fs-6 fw-semibold mt-2 mb-3">{{ __('companies.document_details') }}</div>
                                </div>
                                <!--end::Col-->
                                <!--begin::Col-->
                                <div class="col-xl-9 fv-row">
                        ...
                        <div class="row mb-8" id="reminder_section" style="display: none;">
                            <!--begin::Col-->
                            <div class="col-xl-3">
                                <div class="fs-6 fw-semibold mt-2 mb-3">{{ __('companies.reminder_settings') }}</div>
                            </div>
                            <!--end::Col-->
                            <!--begin::Col-->
                            <div class="col-xl-9 fv-row">
                                ...
                                <div class="row g-9">
                                    ...
                                    <div class="col-md-6 fv-row">
                                        <div class="form-check form-switch form-check-custom form-check-solid">
                                            ...
                                            <label class="form-check-label fw-semibold text-gray-400 ms-3" for="enable_reminder">
                                                {{ __('companies.enable_expiry_reminder') }}
                                            </label>
                                        </div>
                                    </div>
                                    <!--end::Col-->
                                    ...
                                    <div class="col-md-6 fv-row" id="reminder_days_field">
                                        <label class="fs-6 fw-semibold mb-2">{{ __('companies.remind_before_days') }}</label>
                                        ...
                                        <div class="form-text">{{ __('companies.reminder_help_text') }}</div>
                                        @error('reminder_days')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                    ...
                    <div class="card-footer d-flex justify-content-end py-6 px-9">
                        <a href="{{ route('admin.companies.documents.index', $company) }}" class="btn btn-light btn-active-light-primary me-2">
                            {{ __('common.cancel') }}
                        </a>
                        <button type="submit" class="btn btn-primary" id="kt_company_document_submit">
                            <span class="indicator-label">{{ __('companies.save_document') }}</span>
...
// Class definition
var KTCompanyDocumentCreate = function () {
    // Elements
    var form;
    var submitButton;
    ...
    return {
        init: function () {
            form = document.querySelector('#kt_company_document_form');
            submitButton = document.querySelector('#kt_company_document_submit');
